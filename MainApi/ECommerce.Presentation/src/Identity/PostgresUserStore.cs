    ...
    IUserRoleStore<ApplicationUser>
{
    ...
    {
        if (user.NormalizedEmail is null || user.PasswordHash is null) return IdentityResult.Failed();
    ...
    {
        if (user.NormalizedEmail is null) throw new InvalidOperationException("Normalized email is null");

        if (user.PasswordHash is null) throw new InvalidOperationException("Password hash is null");
    ...
    {
        IList<UserEntity>? dbResult = await _userRoleRepository.GetUsersInRoleAsync(roleName);
        List<ApplicationUser> users = new List<ApplicationUser>();

        foreach (UserEntity user in dbResult)
